...
def threshold(M,fac,display,savefig):
    ...
    Inial code checks the input and thus makes the function more robust and 
    user-friendly by making sure the correct type of data is inputted and 
    if not it tells the user what is wrong with their input
    """
    ...
    #saves the figure if the save flag is true
    if savefig == True:
        plt.figure()
        plt.imshow(N)
        plt.title('Yadu Bhageria, threshold, thresholded matrix')
