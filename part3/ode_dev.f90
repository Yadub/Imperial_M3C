...
!$ call omp_set_num_threads(numthreads)
    ...
    t = t0
    ntot = size(y)
!$OMP parallel private(istart,iend,threadID,upbound,lowbound) !add code to this line here as needed
...
!Each thread updates its section of array for every time interval before they all move on to the next time interval
...
function RHS_omp(t,f)
    ...
    RHS_omp = S_adv-c_adv*0.5d0*(f(3:local_n)-f(1:local_n-2))/dx
