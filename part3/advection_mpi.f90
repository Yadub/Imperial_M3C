...
!
!MPI code used with minor changes from solutions for hw5
module advmodule
    implicit none
...
subroutine euler_mpi(comm,numprocs,n,t0,y0,dt,nt,dx,y)
    ...
    N_local = iend - istart + 1 !set size of ylocal for each proc
    ...
    do k = 1,nt
        !-----------------------------------------------------------
        !Send data at top boundary up to next processor for its lower boundary
        !-----------------------------------------------------------
        ...
        !-----------------------------------------------------------
        !Send data at bottom boundary down to previous processor for its upper boundary
        !-----------------------------------------------------------
        if (myid>0) then
            receiver = myid-1
        else
